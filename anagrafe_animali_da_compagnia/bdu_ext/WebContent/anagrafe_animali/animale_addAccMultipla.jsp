...
<jsp:useBean id="veterinariList" class="org.aspcfs.utils.web.LookupList"
	scope="request" />
<jsp:useBean id="veterinariPrivatiList"
...
<script language="JavaScript" TYPE="text/javascript"
	SRC="dwr/interface/Stabilimento.js"> </script>
...
<%
	...
	System.out.println(animale.getIdSpecie());
%>

<%@page import="org.aspcfs.modules.opu.base.Stabilimento"%>
<%@page import="org.aspcfs.modules.opu.base.LineaProduttiva"%><html>
<jsp:useBean id="OperatoreAdded"
...
<%
	...
	boolean stopSubmit = request.getAttribute("stopSubmit")!= null && ((String)request.getAttribute("stopSubmit")).equals("true");
	System.out.println("stopSubmit: " + stopSubmit);
	int idTipologia = -1;
	if (tipologiaSoggetto != null && !"".equals(tipologiaSoggetto))
		idTipologia = Integer.parseInt(tipologiaSoggetto);
	System.out.println("Tipologia del soggetto " + idTipologia + " id operatore "
			+ OperatoreAdded.getIdOperatore());
	if (idTipologia == 1) {
%>
<script>
			function setSoggettoFisico()
			{
				<%
				System.out.println(((LineaProduttiva) (((Stabilimento) OperatoreAdded.getListaStabilimenti().get(0))
						.getListaLineeProduttive().get(0))).getId());
				%>
				document.forms[0].idProprietario.value = '<%=((LineaProduttiva) (((Stabilimento) OperatoreAdded.getListaStabilimenti().get(0))
						.getListaLineeProduttive().get(0))).getId()%>';
...
<script>
			function setSoggettoFisico()
			{
				document.forms[0].idDetentore.value = '<%=((LineaProduttiva) (((Stabilimento) OperatoreAdded.getListaStabilimenti().get(0))
							.getListaLineeProduttive().get(0))).getId()%>';
...
<script>
			function setSoggettoFisico()
			{
				document.forms[0].idVeterinarioMicrochip.value = '<%=((LineaProduttiva) (((Stabilimento) OperatoreAdded.getListaStabilimenti().get(0))
								.getListaLineeProduttive().get(0))).getId()%>';
...
<script>
			function setSoggettoFisico()
			{
				document.forms[0].idOperatoreAdded.value = '<%=((LineaProduttiva) (((Stabilimento) OperatoreAdded.getListaStabilimenti().get(0))
								.getListaLineeProduttive().get(0))).getId()%>';
...
function setSoggettoFisicoDetentore()
{
<%
	String temp = "";
	if(request.getAttribute("opIdDetentore")!=null)
		temp = (String)request.getAttribute("opIdDetentore");		
%>
	document.forms[0].idDetentore.value = '<%=temp%>';
}

	function submit(){
		setSoggettoFisico();
		setSoggettoFisicoDetentore();
...
function pulisciCampo(input){
	...
	(function( $ ) {
	  ...
	  if(input.data( "autocomplete" )!=null && input.data( "autocomplete" )!='undefined')
	  {
...
function checkForm(form) {
	...
	if (document.forms[0].ruolo.value == '24'){
		 if ((form.idSpecie.value == '1' || ((form.idSpecie.value == '2' || form.idSpecie.value == '3'))) && !(form.microchip.value.substring(0, 6) == "380260")){
			message += label("","- Attenzione, MC non valido per un cane: selezionare un Mc del tipo 380260... \r\n");
	       	formTest = false;
	 }
...
<%
    	...
    	{
%>
    	message += label("", "Il canile è in sovraffollamento. Non è piu possibile registrare nuovi cani in ingresso \r\n ");
		formTest = false;
<%
    	}	
%>
    ...
    if (//form.idSpecie.value =='1'  && 
    	    form.idProprietario.value != "-1" && checkCatturaDaProprietarioDetentoreTipo() == false){
    //	message += label("", "Hai selezionato un proprietario di tipo sindaco e un detentore di tipo canile, inserisci cortesemente le informazioni di cattura.");
	//	formTest = false;
        }

/*     if(form.idSpecie.value == '2' &&  form.idProprietario.value != "-1" && checkColoniaPiena() == false){
...
/* 	 if (form.numeroPassaporto != null && form.numeroPassaporto.value!="" && form.numeroPassaporto.value.length<13){
	...
	if (form.flagSindacoFuoriRegione != null && form.flagSindacoFuoriRegione.checked && checkProprietarioSindacoFuoriRegione() == false ){
	  	message += label("", "- Hai selezionato provenienza fuori regione, per favore scegli un proprietario di tipo Sindaco fuori regione.\r\n");
 		formTest = false;
		}


	if (checkProprietarioSindacoFuoriRegione() == true){
		form.flagSindacoFuoriRegione.checked = true;
	}

	
	if (!$('input[name=sesso]:checked').length > 0) {
		message += label("", "- Genere dell'animale richiesto.\r\n");
 		formTest = false;
	}
...
/* function checkColoniaPiena(){
	var ok = false;
	//alert($('#flagCattura').is(':checked'));
	LineaProduttiva.verificaDimensioneColonia(document.addAnimale.idProprietario.value, document.addAnimale.idDetentore.value,  {
		callback:function(data) {
		ok= data;
		},
		timeout:8000,
...
function checkProprietarioSindacoFuoriRegione(){
	var ok = false;
	//alert('f regione');
	//alert($('#flagCattura').is(':checked'));
	LineaProduttiva.verificaProprietarioSindacoFuoriRegione(document.addAnimale.idProprietario.value, {
		callback:function(data) {
		ok= data;
		},
		timeout:8000,
...
%>
		...
		/* 	alert(document.addAnimale.idDetentore.value);
			document.addAnimale.idDetentore.value = -1;
			alert(document.addAnimale.idDetentore.value);
			document.addAnimale.idProprietario.value = -1; */
		}
...
function doCheck(form) {
  ...
  function CheckProvenienzaFuoriRegione() {
  	if (document.addAnimale.flagFuoriRegione != null){
  		if( document.addAnimale.flagFuoriRegione.checked){
		...
		<tr>
			<dhv:evaluate if="<%=(origine == null || origine.equals(""))%>">
				<td width="100%"><a href="AnimaleAction.do"><dhv:label
							name="anagrafica.animale">Animale</dhv:label></a> > <dhv:label
						name="anagrafica.animale.aggiungi">Aggiungi</dhv:label></td>
			</dhv:evaluate>

			<dhv:evaluate if="<%=(origine.equals("operatore"))%>">
				<!--  SE PROVENGO DAl DETTAGLIO OPERATORE LASCIA IL RIFERIMENTO -->
				<td width="100%"><a
					href="OperatoreAction.do?command=Details&opId=<%=opId%>"><dhv:label
							name="anagrafica.animale">Dettaglio Operatore</dhv:label></a> > <dhv:label
						name="anagrafica.animale.aggiungi">Aggiungi animale</dhv:label></td>
			</dhv:evaluate>
		</tr>
	</table>
	<%-- End Trails --%>
...
<body onLoad="abilitaEsitiEhi();abilitaEsitiRik();checkContributo();">
	<form name="addAnimale"
		action="AnimaleAction.do?command=InsertAccMultipla&auto-populate=true<%=addLinkParams(request, "popup|popupType|actionId")%>"
		method="post">
...
%> 		<input type="hidden" name="idTipologiaSoggettoSterilizzante" value="1" id="idTipologiaSoggettoSterilizzante" /> 
		...
		<!-- ID ASL -->
<%	} %>
		<input type="hidden" name="opId" value="<%=((LineaProduttiva)((Stabilimento)OperatoreAdded.getListaStabilimenti().get(0)).getListaLineeProduttive().get(0)).getId()%>">
		<input type="hidden" name="tipologiaSoggetto" value="<%=idTipologia%>">
		<input type="hidden" name="stopSubmit" value="true">
		<input type="hidden" name="idSpecieAccMultipla" value="<%=(String)request.getAttribute("idSpecieAccMultipla")%>">
		<input type="hidden" name="mcAccMultipla" value="<%=(String)request.getAttribute("mcAccMultipla")%>">
		<input type="hidden" name="dataRegistrazioneAccMultipla" value="<%=(String)request.getAttribute("dataRegistrazioneAccMultipla")%>">
		<input type="hidden" name="razzaAccMultipla" value="<%=(String)request.getAttribute("razzaAccMultipla")%>">
		<input type="hidden" name="sessoAccMultipla" value="<%=(String)request.getAttribute("sessoAccMultipla")%>">
		<input type="hidden" name="dataNascitaAccMultipla" value="<%=(String)request.getAttribute("dataNascitaAccMultipla")%>">
		<input type="hidden" name="mantelloAccMultipla" value="<%=(String)request.getAttribute("mantelloAccMultipla")%>">
		<input type="hidden" name="tagliaAccMultipla" value="<%=(String)request.getAttribute("tagliaAccMultipla")%>">
		<input type="hidden" name="dataChippaturaAccMultipla" value="<%=(String)request.getAttribute("dataChippaturaAccMultipla")%>">
		<input type="hidden" name="opIdDetentore" value="<%=(String)request.getAttribute("opIdDetentore")%>">
		...
		%>
		<input type="hidden" name="customerSatisfaction"
			id="customerSatisfaction" value="si">
		<table class="">
			<input type="button"
			...
			<input type="button" value="Salva e Clona" style="display:none;"
		...
		onClick="window.location.href='AnimaleAction.do';this.form.dosubmit.value='false';" /-->
			...
			<dhv:evaluate if="<%=(origine.equals("operatore"))%>">
				...
				<input type="button" style="display:none;"
					value="<dhv:label name="global.button.cancel">Cancel</dhv:label>"
					onClick="window.location.href='OperatoreAction.do?command=Details&opId=<%=opId%>';this.form.dosubmit.value='false';" />
			</dhv:evaluate>
			...
			%>
			<font color="green">Animale con mc <%=animale.getMicrochip()%>
				Salvato Correttamente <dhv:permission
					name="anagrafe_canina-documenti-view">

					<img src="images/icons/stock_print-16.gif" border="0"
						align="absmiddle" height="16" width="16" />
					<a href="#"
						onclick="openRichiesta('<%=animale.getIdAnimale()%>','<%=animale.getIdSpecie()%>');"
						id="" target="_self">Richiesta prima iscrizione</a>
					<dhv:evaluate
						if="<%=(animale.getIdAslRiferimento() != Constants.ID_ASL_FUORI_REGIONE)%>">
						<img src="images/icons/stock_print-16.gif" border="0"
							align="absmiddle" height="16" width="16" />
						<a href="#"
							onclick="openCertificato('<%=animale.getIdAnimale()%>','<%=animale.getIdSpecie()%>');"
						...
						<img src="images/icons/stock_print-16.gif" border="0"
							align="absmiddle" height="16" width="16" />
						<a href="#"
							onclick="openCampioni('<%=animale.getIdAnimale()%>','<%=animale.getIdSpecie()%>');"
							id="" target="_self">Scheda invio campioni</a>
			...
			<table cellpadding="4" cellspacing="0" border="0" width="100%"
				class="details">
				<input type="hidden" value="<%=session.getAttribute("caller")%>"
					name="origineInserimento" id="origineInserimento" />
				<tr>
					<th colspan="2"><strong><dhv:label
								name="accounts.accountasset_include.SpecificInformation">Specific Information</dhv:label></strong>
					</th>
				</tr>
				<%-- <%System.out.println("Ruolo "+User.getRoleId() + " asl"+User.getSiteId() + "asl animale  "+animale.getIdAslRiferimento()); %> --%>
				<dhv:evaluate
					if="<%=(User.getRoleId() != new Integer(ApplicationProperties.getProperty("ID_RUOLO_LLP"))
						...
						&& User.getRoleId() != new Integer(ApplicationProperties.getProperty("ID_RUOLO_HD1")) && User
						.getRoleId() != new Integer(ApplicationProperties.getProperty("ID_RUOLO_HD2"))
				//&& User.getRoleId() != new Integer(ApplicationProperties.getProperty("ID_RUOLO_ANAGRAFE_CANINA"))
				)%>">
					<%-- <%System.out.println("ci entro Ruolo "+User.getRoleId() + " asl"+User.getSiteId() + "asl animale  "+animale.getIdAslRiferimento()); %> --%>
 ...
 %> <%=AslList.getHtmlSelect("idAslRiferimento", animale.getIdAslRiferimento())%> <%
 	} else {
 %> <%=AslList.getSelectedValue(User.getSiteId())%> <input type="hidden"
							size="30" id="idAslRiferimento" name="idAslRiferimento"
 ...
 %>
				...
				<tr id="idSpecie">
					...
					<td><dhv:evaluate if='<%=(animale.getIdProprietario() > 0)%>'>
							<%=specieList.getSelectedValue(animale.getIdSpecie())%>
							<input type="hidden" value="<%=animale.getIdSpecie()%>"
								name="idSpecie" id="idSpecie" />
						</dhv:evaluate> <dhv:evaluate if='<%=!(animale.getIdProprietario() > 0)%>'>
							<%
				...
				<dhv:evaluate
					...
					<tr id="attivitaItinerantetr">
						<td class="formLabel" nowrap><dhv:label name="">Anagrafe effettuata in attività itinerante</dhv:label>
						</td>
						<td><input type="checkbox" id="flagAttivitaItinerante"
							name="flagAttivitaItinerante"
							onclick="inserisciInformazioniAttivitaItinerante()"
							name="flagAttivitaItinerante"
							<%if (animale != null && animale.isFlagAttivitaItinerante()) {%>
							checked="checked" <%}%> />
							<div id="attivitaItinerante"
								<%=(animale != null && animale.isFlagAttivitaItinerante())
						? ("")
						: ("style=\"visibility:hidden\"")%>>
								Comune:
								<%=comuniList.getHtmlSelect("idComuneAttivitaItinerante",
						animale.getIdComuneAttivitaItinerante())%><font color="red">*</font>
								Luogo: <input type="text" name="luogoAttivitaItinerante"
									id="luogoAttivitaItinerante"
									value="<%=animale.getLuogoAttivitaItinerante()%>" />
							</div></td>
					</tr>
				</dhv:evaluate>
				...
				<dhv:evaluate
					...
					<tr class="containerBody">
						<td class="formLabel"><dhv:label name="">Provenienza (Se da Fuori Regione)</dhv:label>
						</td>
						<td><input type="checkbox" id="flagFuoriRegione"
							onclick="CheckProvenienzaFuoriRegione()" name="flagFuoriRegione"
							<%if (animale != null && Cane.isFlagFuoriRegione()) {%>
							checked="checked" <%}%> />
							<div id="utProvenienza"
								<%=(animale != null && Cane.getIdRegioneProvenienza() != -1)
						? ("")
						: ("style=\"visibility:hidden\"")%>>
								<%=regioniList.getHtmlSelect("idRegioneProvenienza", Cane.getIdRegioneProvenienza())%><font color="red">*</font> <input
									type="hidden" id="verDate" name="verDate" value="ok" />
							</div></td>
					</tr>


					<tr class="containerBody" id="provenienza">
						<td class="formLabel">Provenienza (Se Sindaco Fuori Regione)</td>
						<td><input type="checkbox" id="flagSindacoFuoriRegione"
							name="flagSindacoFuoriRegione"
							<%if (Cane != null && Cane.isFlagSindacoFuoriRegione()) {%>
							checked="checked" <%}%>
							onclick="abilitaSelezionePropDet(<%=User.getSiteId()%>)" /></td>
					</tr>
			</dhv:evaluate>
			...
			<% } %>
				...
				<tr class="containerBody" id="proprietario">
					...
					<td>
						<table cellspacing="0" border="0" width="100%" class="details">
							...
							%>
							<dhv:evaluate
								if="<%=(animale != null && animale.getProprietario() != null && animale.getProprietario()
						.getIdOperatore() > 0)%>">
								<%
									idTipologiaLineaProdProprietario = ((LineaProduttiva) (((Stabilimento) animale.getProprietario()
												.getListaStabilimenti().get(0)).getListaLineeProduttive().get(0))).getIdRelazioneAttivita();
										stabProprietario = (Stabilimento) animale.getProprietario().getListaStabilimenti().get(0); //STABILIMENTO PER EVENTULE COMUNE CATTURA
								%>
						...
						? ((LineaProduttiva) (((Stabilimento) animale.getProprietario().getListaStabilimenti().get(0))
								.getListaLineeProduttive().get(0))).getId() : ""%>">


								<!-- Sovrascrivo valore asl con quello del proprietario se il ruolo di chi sta inserendo lo prevede -->
								<dhv:evaluate
									if="<%=Animale.setAslRiferimentoAnimaleByRole(User)%>">
									<input type="hidden" name="idAslRiferimento"
										id="idAslRiferimento"
										value="<%=(animale.getProprietario() != null) ? ((Stabilimento) animale.getProprietario()
							.getListaStabilimenti().get(0)).getIdAsl() : "-1"%>">
							...
							<tr>
								<td><dhv:evaluate
										if="<%=(User.getRoleId() != new Integer(ApplicationProperties.getProperty("ID_RUOLO_LLP")) && User
						.getRoleId() != new Integer(ApplicationProperties.getProperty("UNINA")))%>">
										<a onclick="test();return false;" href="">Ricerca</a>

										<a
											onclick="window.open('OperatoreAction.do?command=Add&tipologiaSoggetto=1&<%=lineaProduttivaFiltro%>&popup=true&idLineaProduttiva=1','','scrollbars=1,width=800,height=600');return false;"
						...
						.getRoleId() == new Integer(ApplicationProperties.getProperty("UNINA")))%>">
							...
							if="<%=(animale.getProprietario() == null || (animale.getProprietario() != null && animale
				...
				<%
					...
					if (animale != null && animale.getDetentore() != null && animale.getDetentore().getIdOperatore() > 0) {
						nomeDetentore = toHtml(animale.getDetentore().getRagioneSociale());
						idTipologiaLineaProduttivaDetentore = ((LineaProduttiva) (((Stabilimento) animale.getDetentore()
								.getListaStabilimenti().get(0)).getListaLineeProduttive().get(0))).getIdRelazioneAttivita();
						idDetentore = ((animale.getDetentore() != null && animale.getDetentore().getListaStabilimenti().size() > 0))
								? ((LineaProduttiva) (((Stabilimento) animale.getDetentore().getListaStabilimenti().get(0))
										.getListaLineeProduttive().get(0))).getId() : -1;
						if ((idTipologiaLineaProdProprietario == LineaProduttiva.idAggregazioneSindaco || idTipologiaLineaProdProprietario == LineaProduttiva.idAggregazioneSindacoFR)
				...
				<dhv:evaluate
					...
					<tr class="containerBody" id="detentore">
						...
						<td>
							...
							.getRoleId() != new Integer(ApplicationProperties.getProperty("UNINA")))%>">
											<a
												onclick="window.open('OperatoreAction.do?command=SearchForm&tipologiaSoggetto=2&idLineaProduttiva1=5&popup=true','','scrollbars=1,width=800,height=600');return false;"
												href="OperatoreAction.do?command=SearchForm&tipologiaSoggetto=2&popup=true&idLineaProduttiva1=5">Ricerca</a>
											<a
												onclick="window.open('OperatoreAction.do?command=Add&tipologiaSoggetto=2&<%=lineaProduttivaFiltro%>&popup=true&idLineaProduttiva=1','','scrollbars=1,width=800,height=600');return false;"
												href="OperatoreAction.do?command=Add&tipologiaSoggetto=2&<%=lineaProduttivaFiltro%>&popup=true&idLineaProduttiva=1">Aggiungi</a>
						...
						href="javascript:popUp('OperatoreAction.do?command=SearchForm&tipologiaSoggetto=2&popup=true');">Ricerca</a>-->
							...
							.getRoleId() == new Integer(ApplicationProperties.getProperty("UNINA")))%>">
											<a
												onclick="window.open('OperatoreAction.do?command=SearchForm&tipologiaSoggetto=2&popup=true','','width=800,height=600');return false;"
												href="OperatoreAction.do?command=SearchForm&tipologiaSoggetto=2&popup=true&idLineaProduttiva1=1">Ricerca</a>
											<a
												onclick="window.open('OperatoreAction.do?command=Add&tipologiaSoggetto=2&<%=lineaProduttivaFiltro%>&popup=true&idLineaProduttiva=1','','scrollbars=1,width=800,height=600');return false;"
												href="OperatoreAction.do?command=Add&tipologiaSoggetto=2&<%=lineaProduttivaFiltro%>&popup=true&idLineaProduttiva=1">Aggiungi</a>
				...
				<tr class="containerBody">
					<dhv:evaluate if='<%=(animale.getIdSpecie() != 3)%>'>
						...
						<td><%=razzaList.getHtmlSelect("idRazza", animale.getIdRazza())%> <%=showAttribute(request, "idRazza")%> <font
							color="red">*</font></td>
					</dhv:evaluate>
					<dhv:evaluate if='<%=(animale.getIdSpecie() == 3)%>'>
						<!--  RAZZA FURETTO -->
				...
				<dhv:evaluate if='<%=(animale.getIdSpecie() == 1)%>'>
					<tr class="containerBody">
						...
						<td><%=tagliaList.getHtmlSelect("idTaglia", Cane.getIdTaglia())%> <%=showAttribute(request, "idTaglia")%> <font
							color="red">*</font></td>
					</tr>
				...
				<tr class="containerBody">
					...
					<td><%=mantelloList.getHtmlSelect("idTipoMantello", animale.getIdTipoMantello())%> <%=showAttribute(request, "idTipoMantello")%>
				...
				<tr class="containerBody">
					...
					<dhv:evaluate
						...
						<td><input type="text" size="16" id="microchip"
							...
							value="<%=(animale.getMicrochip() != null) ? animale.getMicrochip() : ""%>"
					...
					<dhv:evaluate
						...
						<td><input type="text" size="16" id="microchip"
							...
							value="<%=(animale.getMicrochip() != null) ? animale.getMicrochip() : ""%>"
							onchange="verificaInserimentoAnimale(document.forms[0].microchip)"
							readonly="readonly"> <font color="red">*</font> <%=showError(request, "ErroreMicrochip")%>&nbsp;&nbsp;
				...
				<tr class="containerBody" style="">
					...
					<td><dhv:evaluate
						...
						.getRoleId() == new Integer(ApplicationProperties.getProperty("UNINA")))%>">
							...
							<input type="hidden" name="idVeterinarioMicrochip"
								id="idVeterinarioMicrochip" value="<%=User.getUserId()%>" />
							...
							<table cellspacing="0" border="0" width="100%" class="details">
								<tr>
									<td><dhv:evaluate
											...
											if='<%=(animale.getIdSpecie() == Gatto.idSpecie)%>'>
											<%=veterinariList.getHtmlSelect("idVeterinarioMicrochip", Gatto.getIdVeterinarioMicrochip())%>
										</dhv:evaluate>
										<dhv:evaluate
											if='<%=(animale.getIdSpecie() == Furetto.idSpecie)%>'>
											<%=veterinariList.getHtmlSelect("idVeterinarioMicrochip",
							Furetto.getIdVeterinarioMicrochip())%>
										</dhv:evaluate></td>
								</tr>
							</table>
							...
							<td><input type="checkbox" id="flagMorsicatore"
								...
								<div id="mors">
									<br> Date: <input readonly type="text" id="dataMorso"
										...
										value="<%=(Cane.getDataMorso() != null) ? toDateasString(Cane.getDataMorso()) : ""%>"
				...
				<tr class="containerBody" id="sterilizzazione">
					...
					<td>
						...
						.getRoleId() != new Integer(ApplicationProperties.getProperty("UNINA")))%>">
											<dhv:evaluate
												if="<%=(animale.getIdSpecie() != Furetto.idSpecie)%>">
						Contributo Regionale <input type="checkbox"
										...
										<div id="pratica_contributo" name="pratica_contributo"
											style="visibility: hidden">
											<input type="hidden" name="oldPratica"
												value="<%=request.getParameter("pratica") != null ? request.getParameter("pratica") : ""%>">
										...
										<input type="hidden" name="contributoPagato">
									</div>
				...
				<dhv:evaluate
					if="<%=(User.getRoleId() != 24 && User.getRoleId() != Integer.valueOf(ApplicationProperties
						.getProperty("UNINA")))%>">
	 ...
	 --%>
					<tr class="containerBody">
						...
						<td><input type="text" name="numeroPassaporto" size="15"
							...
							onchange="javascript:verificaPassaporto(this);"></td>
					</tr>

					<tr class="containerBody">
						<td class="formLabel"><dhv:label name="">Data rilascio passaporto - Data scadenza passaporto</dhv:label>
						</td>
						<td><input readonly type="text" name="dataRilascioPassaporto"
							id="dataRilascioPassaporto" size="10"
							...
							readonly name="dataScadenzaPassaporto"
							id="dataScadenzaPassaporto" size="10" value=""
		...
		<dhv:evaluate if='<%=(animale.getIdSpecie() == 1)%>'>
			<tr class="containerBody">
				...
				<td><input readonly type="text" name="dataVaccino" size="10"
					...
					nomecampo="dataVaccino" tipocontrollo="T10,T11,T9"
      ...
      <%}else{ <input type="text" size="10"
					...
					value="<%=toHtmlValue(animale.getNumeroLottoVaccino())%>"></td>
			</tr>
		...
		<dhv:evaluate if='<%=(animale.getIdSpecie() == 1)%>'>
			<tr class="containerBody">
				...
				<td><input type="text" size="10"
					...
					value="<%=toHtmlValue(animale.getNomeVaccino())%>"></td>
			</tr>
	...
	<dhv:evaluate
			...
			<tr class="containerBody">
				...
				<td><input type="text" size="10"
					...
					value="<%=toHtmlValue(animale.getProduttoreVaccino())%>"></td>
			</tr>
	</dhv:evaluate>
	...
	<dhv:evaluate
		...
		<dhv:evaluate if='<%=(animale.getIdSpecie() == 1)%>'>
			<tr class="containerBody">
				...
				<td><input readonly type="text" name="dataScadenzaVaccino" size="10"
					...
					nomecampo="dataScadenzaVaccino" tipocontrollo="T10,T11,T9"
			...
			<dhv:evaluate
						if="<%=(User.getRoleId() != new Integer(ApplicationProperties.getProperty("ID_RUOLO_LLP")))%>">
			<tr class="containerBody">
				<td class="formLabel"><dhv:label name="">Veterinario esecutore vaccinazione</dhv:label>
				</td>
				<td><%=veterinariPrivatiList.getHtmlSelect(
									"idVeterinarioEsecutoreAccreditato", -1)%></td>
			</tr>
		</dhv:evaluate>
		
		<dhv:evaluate
						if="<%=(User.getRoleId() == new Integer(ApplicationProperties.getProperty("ID_RUOLO_LLP")))%>">
				<input type="hidden" name="idVeterinarioEsecutoreAccreditato" value="<%=User.getUserId() %>" id="idVeterinarioEsecutoreAccreditato"	/>
		...
		<dhv:evaluate if='<%=(animale.getIdSpecie() == Gatto.idSpecie)%>'>
			<tr class="containerBody">
				...
				<td> <input type="text" size="10"
					name="numeroLottoVaccino" maxlength="15"
					value="<%=toHtmlValue(Gatto.getNumeroLottoVaccino())%>"></td>
			</tr>
...
<!-- ------------------------------------------------------------------------------------------------------------ -->
			...
			<dhv:evaluate if="<%=(User.getRoleId() != 24)%>">
				...
				<dhv:evaluate
					if="<%=!(User.getRoleId() == Integer.valueOf(ApplicationProperties.getProperty("UNINA")))%>">
					<dhv:evaluate if='<%=(animale.getIdSpecie() == 1)%>'>
						<table cellpadding="4" cellspacing="0" border="0" width="100%"
							class="details">
							<tr>
									...
									<table class="noborder">
										<tr>
											<td>Ord.sind. <input type="text" size="10"
												...
												value="<%=toString(Cane.getLeishmaniosiNumeroOrdinanzaSindacale())%>">
												del <input readonly type="text"
												id="leishmaniosiDataOrdinanzaSindacale"
												name="leishmaniosiDataOrdinanzaSindacale" size="10"
												value="<%=toDateasString(Cane.getLeishmaniosiDataOrdinanzaSindacale())%>"
									...
									<table class="noborder">
										<tr>
											<td><input type="checkbox" id="flagControlloEhrlichiosi"
												<%if (Cane.isFlagControlloEhrlichiosi()) {%>
												checked="checked" <%}%> name="flagControlloEhrlichiosi"
												...
												<table class="noborder">
													<tr>
														<td>Data <input readonly type="text"
															...
															NAME="anchor19" ID="anchor19"> <img
																src="images/icons/stock_form-date-field-16.gif"
    	...
    	<%}else{ --%> <%=esitoList.getHtmlSelect("esitoControlloEhrlichiosi",
								Cane.getEsitoControlloEhrlichiosi())%> <%-- }--%></td>
													</tr>
												</table>
											</td>
									...
									<table class="noborder">
										<tr>
											<td><input type="checkbox"
												id="flagControlloRickettsiosi"
												name="flagControlloRickettsiosi"
												<%if (Cane.isFlagControlloRickettsiosi()) {%>
												checked="checked" <%}%> onclick="abilitaEsitiRik();" /></td>
											<td id="esitoRickadd1">
												<table class="noborder">
													<tr>
														<td>Data <input readonly type="text"
														...
														<td id=>Esito <%=esitoList.getHtmlSelect("esitoControlloRickettsiosi",
								Cane.getEsitoControlloRickettsiosi())%></td>
													</tr>
												</table>
											</td>
										</tr>
					...
					<dhv:evaluate if='<%=(animale.getIdSpecie() != Furetto.idSpecie)%>'>
						<table id="cattura" name="cattura" cellpadding="4" cellspacing="0"
							border="0" width="100%" class="details">
							<tr>
							...
							<tr class="containerBody">
								...
								<td>
									...
									id="flagCattura" name="flagCattura" />
								</td>
							</tr>
							...
							<tr class="containerBody">
								...
								<input type="hidden" name="idComuneCattura" id="idComuneCattura"
									value="<%=idComuneCattura%>"
									onChange="javascript:visualizzaMunicipalita();" />
								<td>
									<%-- if (!update) { --%> <%=comuniList.getSelectedValue(idComuneCattura)%>
									<%-- } else { %>
    ...
    <% } --%>
							...
							<tr class="containerBody">
								...
								<td>
									<%-- if (!update) { --%> <textarea name="verbaleCattura"
   ...
   <%} --%>
			...
			<div id="dialogMunicipalita"
				title="Municipalit&agrave; del comune di Napoli">
			...
			function gestioneOrigineDopoReloadPagina(){
			 ...
			 function mostraOrigine(){
				 ...
				 if($("#dataNascita").val()!='')
				 {
				 	...
				 	if(idSpecie==null)
					 	idSpecie=document.forms[0].idSpecie.value;
	 ...
	 function test(){
		//	alert('aaa');
			...
			win = window.open('OperatoreAction.do?command=SearchForm&tipologiaSoggetto=1&<%=lineaProduttivaFiltro%>&popup=true','','width=800,height=600');
