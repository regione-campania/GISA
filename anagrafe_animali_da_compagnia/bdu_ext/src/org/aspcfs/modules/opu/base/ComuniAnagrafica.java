...
public void setCodice(String codice) {
  ...
  public ArrayList<ComuniAnagrafica> buildList(Connection db, int asl,int tipoSelezione) throws SQLException {
	    ...
	    else
	    {
	    	...
	    	{
	    		sql.append("SELECT id,nome FROM comuni1  where  cod_regione != '"+CODICE_REGIONE_CAMPANIA+"' and trashed is false order by nome");

	    	}
	    	else
	    	{
	    		if(asl!=-1)
		    		sql.append("SELECT id,nome FROM comuni1 c , lookup_site_id asl WHERE c.codiceistatasl =asl.codiceistat and asl.code = ? and trashed is false union SELECT id,nome FROM comuni1  where  cod_regione != '"+CODICE_REGIONE_CAMPANIA+"' and trashed is false order by nome");
		    	else
		    		sql.append("SELECT id,nome FROM comuni1 c left join lookup_site_id asl on c.codiceistatasl =asl.codiceistat and trashed is false order by nome");
...
public ArrayList<ComuniAnagrafica> getComuni (Connection db , int idProvincia)
{
	...
	String sql = "select id,nome from comuni1 where and trashed is false  ";
	if (idProvincia !=-1)
	{
...
public static HashMap<String, Integer>  getProvinciaRegioneDaIdComune(Connection db, int idComune){
	...
	try{
		PreparedStatement pst = db.prepareStatement("select cod_provincia, cod_regione from comuni1 where trashed is false and id = ?");
