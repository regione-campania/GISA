...
<%
Iterator itCu = listaLinee.iterator();
while (itCu.hasNext()){
LineaVariazione linea =(LineaVariazione) itCu.next();
...
<col width="15%"/>
<tr>
<th colspan="6">Variazione su impresa</th>
</tr>
...
<tr class="row<%=i%2%>"><td colspan="6"><u><%=OrgDetails.getName() %></u></td></tr> 
<tr class="row<%=i%2%>">
<td><b>Stato</b></td><td><%=ListaStati.getSelectedValue(lineaOriginale.getStato()) %></td>  
...
<%for (int j=0; j<listaCu.size(); j++){
	int idcu = listaCu.get(j);%>
...
<tr><td colspan="6"><input type="button" id="conferma" value="conferma" onClick="checkForm(this.form, 1)"/></td></tr>
</table>
