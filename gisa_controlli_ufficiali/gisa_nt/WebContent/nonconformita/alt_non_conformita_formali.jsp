      ...
      <table border="0" width="100%" cellspacing="0" cellpadding="10" class="empty" >
        ...
        <tr id = "nc_formali_1" >
         ...
         <%=ProvvedimentiBenessereTrasporto.getHtmlSelect("ProvvedimentiBenessereTrasporto1_1", -1)%>
          ...
          <select id="OperatoreMercato1_1" name="OperatoreMercato1_1">
...
<a class="ovalbutton" href = "javascript: if (document.getElementById('abilita_formali').value == 'true' && document.getElementById('descrizione_or_combo_sel_formali').value == 'false' && trim(document.getElementById('valutazione_rischio_formali').value) != 'INSERIRE LA VALUTAZIONE DEL RISCHIO DELLE NON CONFORMITA\' FORMALI RISCONTRATE' && trim(document.getElementById('valutazione_rischio_formali').value) != '') { openNotePopupFollowup('<%=CU.getAltId() %>','<%=CU.getId() %>','<%=CU.getAssignedDate() %>',1) }else{ alert('Controllare che per ogni non conformita\' aggiunta sia stato selezionato un tipo , e inserita la relativa descrizione, e compilata la valutazione del rischio !') }" onclick="return !this.disabled;" ><span >Inserisci Follow Up</span></a>
