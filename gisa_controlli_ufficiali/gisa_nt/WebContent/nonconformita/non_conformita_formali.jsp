...
<a class="ovalbutton" href = "javascript: if (document.getElementById('abilita_formali').value == 'true' && document.getElementById('descrizione_or_combo_sel_formali').value == 'false' && trim(document.getElementById('valutazione_rischio_formali').value) != 'INSERIRE LA VALUTAZIONE DEL RISCHIO DELLE NON CONFORMITA\' FORMALI RISCONTRATE' && trim(document.getElementById('valutazione_rischio_formali').value) != '') { openNotePopupFollowup('<%=CU.getOrgId() %>','<%=CU.getId() %>','<%=CU.getAssignedDate() %>',1) }else{ alert('Controllare che per ogni non conformita\' aggiunta sia stato selezionato un tipo , e inserita la relativa descrizione, e compilata la valutazione del rischio !') }" onclick="return !this.disabled;" ><span >Inserisci Follow Up</span></a>
