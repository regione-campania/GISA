 ...
 * @type String
 **/
var _cookiePrefix = "jQu3ry_3teps_St@te_";
...
{
    return paginationClickNext(wizard, options, state, increaseCurrentIndexBy(state, 1));
}
...
{
    return paginationClickPrev(wizard, options, state, decreaseCurrentIndexBy(state, 1));
}
...
function goToStep(wizard, options, state, index)
{
    ...
    if (state.currentIndex==3)
	{
        ...
        if (state.currentIndex==3 &&  docCompleta!="0")	
    	{
        	 var currentOrNewStepAnchor1 = getStepAnchor(wizard, 0);
        	 var currentOrNewStepAnchor2 = getStepAnchor(wizard, 1);
        	 //var currentOrNewStepAnchor3 = getStepAnchor(wizard, 2);
...
function paginationClickPrev(wizard, options, state, index)
{
    ...
    if ( docCompleta=="0")
    	{
    if (index >= 0 && (state.currentIndex > 0 && ( state.currentIndex!=3 ))  && index < state.stepCount && !(options.forceMoveForward && index < state.currentIndex))
    {
        ...
        if(oldIndex == 2)
        {
        	...
        	resetInBulkTutteLineeCandidatiRanked();
        }
    	...
    	if (state.currentIndex==2 &&  docCompleta!="0")	
    	{
...
function paginationClickNext(wizard, options, state, index)
{
 ...
 var docCompleta = document.getElementById("pratica_completa").value;
    ...
    if ((state.currentIndex!=2 &&  docCompleta=="0") || (state.currentIndex==2 &&  docCompleta=="1"))
    	{
    ...
    if (index >= 0 && index < state.stepCount && !(options.forceMoveForward && index < state.currentIndex))
    {
    	...
    	alert("Attenzione ! PER PROSEGUIRE E' NECESSARIO SALVARE LA PRATICA CLICCANDO SULL'APPOSITO PULSANTE.")
    	}
...
function saveDef()
{
    showAlert('SALVATAGGIO DEFINITIVO IN COSTRUZIONE');
}
...
function salvaTemporanea(wizard, options, state, index)
{
    ...
    var indice = 0 ;
    salva = true ;
    /*non sono piu' necessari file allegati */
    /*
	    while (document.getElementById("allegato"+indice)!=null)
	    	{
	    	if (document.getElementById("allegato"+indice).value=="0")
	    		{
	    		salva=false ;
	    		}
	    	indice++;
    ...
    */
    if(document.getElementById('superficie')!=null && (document.getElementById('superficie').value=='0' || document.getElementById('superficie').value==''))
    {
    	alert('ATTENZIONE! SPECIFICARE LA SUPERFICIE.');
    	return true;
    }
    if (salva==true && index >= 1 && index < state.stepCount && !(options.forceMoveForward && index < state.currentIndex))
    {
        var anchor = getStepAnchor(wizard, index),
      ...
      //previous._enableAria(false);
    	...
    	alert('ATTENZIONE! PRATICA GIA\' SALVATA.');
    }
        return true;
    }
    else
...
function refreshPagination(wizard, options, state)
{
    ...
    if (options.enablePagination)
    {
        ...
        if (!options.forceMoveForward )
        {
            ...
            previous._enableAria(state.currentIndex > 0 && state.currentIndex!=3);
        }

        
     	var save = wizard.find(".actions a[href$='#saveTemp']").parent();
      	 save._showAria(state.currentIndex ==2);
          
          var previous = wizard.find(".actions a[href$='#previous']").parent();
          previous._showAria(state.currentIndex >0);	 
          
        
//        var previousSave = wizard.find(".actions a[href$='#saveTemp']").parent();
//        //previousSave._enableAria(state.currentIndex>2);
//        //previousSave.addClass("disabled");
//        var back = wizard.find(".actions a[href$='#previous']").parent();
...
//            previous._showAria(state.stepCount === 0 || state.stepCount > (state.currentIndex + 1)).
//            _enableAria(state.currentIndex > 1);
//            back._showAria(state.stepCount === 0 || state.stepCount > (state.currentIndex + 1))
//            
//        }
    }
}
...
var defaults = $.fn.steps.defaults = {
    ...
    labels: {
         ...
         * @for defaults
         **/
        finish: "Stampa Numero Protocollo",
