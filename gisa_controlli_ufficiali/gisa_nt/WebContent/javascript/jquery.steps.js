    ...
    if (index >= 0 && (state.currentIndex > 0 && ( state.currentIndex!=5 ))  && index < state.stepCount && !(options.forceMoveForward && index < state.currentIndex))
    {
    	...
    	if (state.currentIndex==5 &&  docCompleta!="0")	
    	{
    		if ( praticaCompleta=='0')
    			{
    			alert("ATTENZIONE ! DOPO AVER SALVATO LA PRATICA NON è PIU POSSIBILE TORNARE INDIETRO.ANDARE AVANTI PER VISUALIZZARE IL RIEPILOGO.");
    			}
    		else
...
function paginationClickNext(wizard, options, state, index)
{
 ...
 var docCompleta = document.getElementById("pratica_completa").value;
    ...
    if ((state.currentIndex!=5 &&  docCompleta=="0") || (state.currentIndex==5 &&  docCompleta=="1"))
    	{
...
function salvaTemporanea(wizard, options, state, index)
{
    ...
    var indice = 0 ;
    salva = true ;
    /*nono sono piu' necessari file allegati */
...
function refreshPagination(wizard, options, state)
{
    ...
    if (options.enablePagination)
    {
        ...
        previous._enableAria(state.currentIndex >4);
        ...
        else
        {
            ...
            previous._showAria(state.stepCount === 0 || state.stepCount > (3)).
            _enableAria(state.currentIndex > 3);
