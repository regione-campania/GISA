  ...
  %>
   ...
   <td width="10%" valign="top" nowrap>
		...
		<% if(thisTic.getIdStabilimento() > 0) {%>
			<a href="<%=OrgDetails.getAction() %>Vigilanza.do?command=TicketDetails&id=<%= thisTic.getId() %>&stabId=<%= thisTic.getIdStabilimento()%>&idStabilimentoopu=<%= thisTic.getIdStabilimento()%><%= addLinkParams(request, "popup|popupType|actionId") %>"><zeroio:tz timestamp="<%= thisTic.getAssignedDate() %>" dateOnly="true" timeZone="<%= thisTic.getAssignedDateTimeZone() %>" showTimeZone="false" default="&nbsp;"/>
		<% } else { 
			thisTic.setIdStabilimento(thisTic.getIdApiario());
		%>
		<a href="<%=OrgDetails.getAction() %>Vigilanza.do?command=TicketDetails&id=<%= thisTic.getId() %>&stabId=<%= thisTic.getIdStabilimento()%>&idStabilimentoopu=<%= thisTic.getIdStabilimento()%><%= addLinkParams(request, "popup|popupType|actionId") %>"><zeroio:tz timestamp="<%= thisTic.getAssignedDate() %>" dateOnly="true" timeZone="<%= thisTic.getAssignedDateTimeZone() %>" showTimeZone="false" default="&nbsp;"/>
		<% } %>
	   </a>
