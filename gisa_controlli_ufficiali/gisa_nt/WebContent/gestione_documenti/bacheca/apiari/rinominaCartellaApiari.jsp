...
function checkFormRinominaCartella(formRinominaCartella) {
	...
	<form name="rinominaCartella" action="GestioneBacheca.do?command=GestisciCartellaApiari" method="post">
	...
	<input type="button"
		value="Annulla"
		onClick="window.location.href='GestioneBacheca.do?command=ListaApiari&storeId=<%=request.getParameter("storeId")%>&folderId=<%=request.getParameter("folderId")%>&parentId=<%=request.getParameter("parentId")%>';" />
