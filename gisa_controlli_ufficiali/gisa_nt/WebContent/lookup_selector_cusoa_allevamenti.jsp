...
%>
<form name="elementListView" method="post" action="LookupSelector.do?command=PopupSelectorSOA">
<br />
...
<table cellpadding="4" cellspacing="0" border="0" width="100%" class="pagedList">
  <tr>
    ...
    <th width="70%">
      <dhv:label name="">Indirizzo sede Operativa</dhv:label>
    </th>
  ...
  <tr class="row<%= 1 %>">
    <td valign="center">
      <a href="javascript:setParentValue2_cusoaFuoriRegione_allevamenti('<%= "org"+idasl %>','<%= StringUtils.jsStringEscape("Fuori Regione") %>','<%=idasl %>','<%=StringUtils.jsStringEscape("Fuori Regione")+ " , " +"" + "," +""%>','<%="-1" %>' );">
        <%= "SOA Fuori Regione" %>
      </a>
    </td>
    <td valign="center">
  ...
  <tr class="row<%= rowid %>">
    <td valign="center">
      <a href="javascript:setParentValue2_cusoa_allevamenti('<%= "org"+idasl %>','<%= StringUtils.jsStringEscape(ragioneSociale) %>','<%=idasl %>','<%=StringUtils.jsStringEscape(indirizzo)+ " , " + StringUtils.jsStringEscape(citta) + "," + StringUtils.jsStringEscape(prov)%>','<%=orgId %>' );">
