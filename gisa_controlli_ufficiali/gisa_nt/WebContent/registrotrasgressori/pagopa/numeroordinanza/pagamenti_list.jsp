...
<%-- <% if (p.getIdentificativoUnivocoVersamento()!=null && (p.getStatoPagamento()==null || (!Pagamento.PAGAMENTO_COMPLETATO.equalsIgnoreCase(p.getStatoPagamento()) && !Pagamento.PAGAMENTO_IN_CORSO.equalsIgnoreCase(p.getStatoPagamento())))) { %> <dhv:permission name="gestione_pagopa-edit"><input type="button" value="AGGIORNA RICHIESTA DI PAGAMENTO"  onClick="aggiorna('<%= p.getId()%>', this.form)"/ > <br/> <font color="green">Mediante questa funzione, la data del pagamento sarà aggiornata in PagoPa.</font></dhv:permission> <% } %> --%>
</td></tr>
...
<%if (p.getIdentificativoUnivocoRiscossione()!=null && !"".equals(p.getIdentificativoUnivocoRiscossione())) { %><br/>
<b>Identificativo univoco riscossione: </b> <%=p.getIdentificativoUnivocoRiscossione() %> <br/>
