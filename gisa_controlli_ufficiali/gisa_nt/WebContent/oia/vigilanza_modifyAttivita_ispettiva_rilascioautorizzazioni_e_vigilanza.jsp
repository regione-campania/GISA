...
<form name="details" action="OiaVigilanza.do?command=UpdateTicket&auto-populate=true<%= addLinkParams(request, "popup|popupType|actionId") %>&orgId=<%=OrgDetails.getOrgId()  %>" onSubmit="return checkForm(this);" method="post">
...
<td>
  <a href="Oia.do?command=SearchForm">Autorità Competenti</a> >
  <a href="Oia.do?command=Details&idNodo=<%=OrgDetails.getOrgId()  %>&orgId=<%=OrgDetails.getOrgId()%>">Scheda Dipartimento</a> >
  <a href="OiaVigilanza.do?command=ViewVigilanza&idNodo=<%=OrgDetails.getOrgId()  %>&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="vigilanza">Tickets</dhv:label></a> >
  <% if (request.getParameter("return") == null) {%>
  <a href="OiaVigilanza.do?command=TicketDetails&idNodo=<%=OrgDetails.getOrgId()  %>&id=<%=TicketDetails.getId()%>"><dhv:label name="campioni.dettagli">Scheda Controllo Ufficiale</dhv:label></a> >
  <%}%>
     ...
     <dhv:evaluate if="<%= !TicketDetails.isTrashed() %>" >
      <dhv:evaluate if="<%= TicketDetails.getClosed() != null %>" >
          <dhv:permission name="oia-oia-vigilanza-edit">
            <input type="submit" value="<dhv:label name="button.reopen">Reopen</dhv:label>" onClick="javascript:this.form.action='OiaVigilanza.do?command=ReopenTicket&id=<%=TicketDetails.getId()%><%= addLinkParams(request, "popup|popupType|actionId") %>'">
           </dhv:permission>
           <dhv:evaluate if='<%= "list".equals(request.getParameter("return"))%>' >
            <input type="submit" value="Annulla" onClick="javascript:this.form.action='Distributori.do?command=ViewVigilanza&orgId=<%=OrgDetails.getOrgId()%><%= addLinkParams(request, "popup|popupType|actionId") %>&idNodo=<%=OrgDetails.getOrgId()  %>'" />
           </dhv:evaluate>
           <dhv:evaluate if='<%= !"list".equals(request.getParameter("return"))%>' >
            <input type="submit" value="Annulla" onClick="javascript:this.form.action='OiaVigilanza.do?command=TicketDetails&id=<%= TicketDetails.getId() %><%= addLinkParams(request, "popup|popupType|actionId") %>&idNodo=<%=OrgDetails.getOrgId()  %>'" />
           </dhv:evaluate>
      </dhv:evaluate>
      <dhv:evaluate if="<%= TicketDetails.getClosed() == null %>" >
          <dhv:permission name="oia-oia-vigilanza-edit">
           ...
           <dhv:evaluate if='<%= "list".equals(request.getParameter("return"))%>' >
            <input type="button" value="Annulla" onClick="javascript:this.form.action='Distributori.do?command=ViewVigilanza&orgId=<%=OrgDetails.getOrgId()%><%= addLinkParams(request, "popup|popupType|actionId") %>&idNodo=<%=OrgDetails.getOrgId()  %>'; this.form.submit();" />
           </dhv:evaluate>
           <dhv:evaluate if='<%= !"list".equals(request.getParameter("return"))%>' >
            <input type="button" value="Annulla" onClick="javascript:this.form.action='OiaVigilanza.do?command=TicketDetails&id=<%= TicketDetails.getId() %><%= addLinkParams(request, "popup|popupType|actionId") %>&idNodo=<%=OrgDetails.getOrgId()  %>'; this.form.submit();" />
           </dhv:evaluate>
