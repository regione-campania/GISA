...
/**
  ...
  public String getHtmlText(String thisSelectName, String thisDefaultValue) {
	    selectName = thisSelectName;
	    setDefaultValue(thisDefaultValue);
	    if (!built) {
	      buildAsText();
  ...
  public String getHtmlDb(String thisSelectName, int thisDefaultKey,boolean obbligatorio,String label) {
	    selectName = thisSelectName;
	    setDefaultKey(thisDefaultKey);
	    if (!built) {
	      build();
	    }
	    return getHtmlDb(obbligatorio,label);
	  }
  ...
  public String getHtml() {
    ...
    if (!checkboxOutput) {
    	if (searchable==true)
    		 outputHtml.append("<p "+   (selectStyle == null ? "" : "style='" + selectStyle + "' ") +"><b>Ricerca</b></p><input type = 'text' "+   (selectStyle == null ? "" : "style='" + selectStyle + "' ") +" name = '"+this.selectName +"Search' id = '"+this.selectName +"Search' onchange='searchFromLookup('"+tableName+"','"+ this.selectName+"')'></br>");
      outputHtml.append(
  ...
  public String getHtmlDb(boolean obbligatorio,String label) {
	    //Build the html for the results
	    if (!built) {
	      build();
	    }
	    ...
	    if (!checkboxOutput) {
	      outputHtml.append(
	          "<select size='" + this.selectSize + "' label='"+label+"'" +
	        		  (obbligatorio == true ? "obbligatorio='si'" : "obbligatorio ='no'") +
	          (selectStyle == null ? "" : "style='" + selectStyle + "' ") +
	          "name='" + this.selectName + "'" +
	          (jsEvent != null ? " " + this.jsEvent : "") +
	          (idName != null ? " " + "id='" + this.idName + "' " : "") +
	    ...
	    if(obbligatorio==true)
	    	  outputHtml.append("<font color = 'red'>*</font>");
  ...
  public void addItem(int tmp1, String tmp2) {
	  ...
	  if(required==true && tmp1 ==-1)
	  {
		  this.add(new HtmlOption("", tmp2));
	  }
	  else
		  this.add(new HtmlOption(String.valueOf(tmp1), tmp2));
  }
  ...
  public void addItem(int tmp1, int tmp2) {
	    this.add(new HtmlOption(String.valueOf(tmp1), String.valueOf(tmp2)));
	  }
  ...
  public void build() {
    ...
    if (!this.firstEntry.equals("")) {
      if (((this.defaultValue.equals("")) && (this.defaultKey.equals(""))) ||
        ...
        if (!checkboxOutput) {
          rowList.append(
              "<option selected='selected' value='" + 0 + "'>" + this.firstEntry + "</option>");
        } else {
          rowList.append(
    ...
    for (int i = 0; i < this.size(); i++) {
      ...
      if (thisOption.isGroup()) {
        ...
        rowList.append("<optgroup label=\"" + tmp2 + "\" title='"+toHtml(
                tmp2)+"'>");
      } else {
        ...
        if (!checkboxOutput) {
        	String option ="";
        	  ...
        	  if(thisOption.getAttributeList()!=null && thisOption.getAttributeList().get("permesso")!=null && !"".equals(thisOption.getAttributeList().get("permesso")))
        			  {
        		  option="<dhv:permission name=\""+thisOption.getAttributeList().get("permesso")+"\">";
        		  
        			  }
        	  
        	//System.out.println("sono entrato nell'if valore di tmp1 : "+tmp1);
        	
        	if(tmp1.equals("-1")){
        		
        		if(this.getTableName().equalsIgnoreCase("lookup_opu_tipo_impresa"))
        		{
        			option += "<option " + optionSelected + "value='' " + attributes + "> " + 
        	        		
        		toHtml(tmp2).toUpperCase() + "</option>";
        		}
        		else
        			option += "<option " + optionSelected + "value='" + tmp1 + "' " + attributes + "> " + 
        	...
        	{
        		 option += "<option " + optionSelected + "value='" + tmp1 + "'  " + attributes + ">" + toHtml(
                         tmp2) + "</option>";
        	}
        	  if(thisOption.getAttributeList()!=null && thisOption.getAttributeList().get("permesso")!=null && !"".equals(thisOption.getAttributeList().get("permesso")))
        			  {
        		  option+="</dhv:permission>";
        		  
        			  }
        	rowList.append(option);
          /*rowList.append(
  ...
  public void buildAsText() {
	    ...
	    for (int i = 0; i < this.size(); i++) {
	      ++processedRowCount;
	      HtmlOption thisOption = (HtmlOption) this.get(i);
	      String tmp1 = thisOption.getText();
	      String tmp2 = thisOption.getText();
	      String attributes = thisOption.getAttributes();
	      if (thisOption.isGroup()) {
	        if (groupOpen) {
	        ...
	        if (!checkboxOutput) {
	        	//System.out.println("sono entrato nell'if valore di tmp1 : "+tmp1);
	        	String option = "<option " + optionSelected + "value=\"" + tmp1.toUpperCase() + "\"  " + attributes + ">" + toHtml(
	                    tmp2) + "</option>";
	        	if(tmp1.equals("-1")){
