 ...
 * 
 * @author mrajkowski
 * @version $Id: Address.java 16202 2006-10-06 21:06:27Z matt $
...
public class Address implements Serializable {
  ...
  public String getStreetAddressLine1() {
    return (streetAddressLine1!=null) ? streetAddressLine1 : "";
  }
  ...
  public String toString() {
    ...
    if (this.getStreetAddressLine2() != null
          ...
          thisAddress.append("C/O : "+this.getStreetAddressLine2().trim() + "\r\n");
        }
    ...
    if (this.getLatitude() != 0) {
      thisAddress.append("Latitudine: " + this.getLatitude() + "\r\n");
    }
    if (this.getLongitude() != 0) {
      thisAddress.append("Longitudine: " + this.getLongitude() + "\r\n");
    }
  ...
  public void buildRecord(ResultSet rs) throws SQLException {
    ...
    if (verifica_coordinate_allevamenti(rs) == true)
    {
    	this.setLatitude(rs.getDouble("latitudine_wgs84_utm33n"));
        this.setLongitude(rs.getDouble("longitudine_wgs84_utm33n"));
    }
    else
    {
  ...
  private boolean verifica_coordinate_allevamenti(ResultSet rs) throws SQLException
  {
	  boolean trovato = false ;
	  int numCol = rs.getMetaData().getColumnCount();
	  for (int i = 1; i <= numCol; i++)
	  {
		  if (rs.getMetaData().getColumnName(i).equals("latitudine_wgs84_utm33n"))
		  {
			  trovato = true ;
			  break;
		  }
  ...
  public void buildRecord2(HttpServletRequest request, int parseItem) {
	    this.setType(request.getParameter("address4type"+parseItem));
	    if (request.getParameter("address" + 4 + "id"+parseItem) != null) {
	      this.setId(request.getParameter("address" + 4 + "id"+parseItem));
	    }
	    String buffer = null;
	   
	    	 buffer = request.getParameter("address" + 4 + "line1"+parseItem);
	    ...
	    {
	    	  this.setStreetAddressLine1("");
	    }
	    buffer = request.getParameter("address" + 4 + "line2"+parseItem);
	    if (buffer != null ) {
	      this.setStreetAddressLine2(buffer);
	    }
	    else
	    {
	    	this.setStreetAddressLine2("");
	    }
	    buffer = request.getParameter("address" + 4 + "line3"+parseItem);
	    if (buffer != null ) {
	      this.setStreetAddressLine3(buffer);
	    }
	    else
	    {
	    	this.setStreetAddressLine3("");
	    }
	    buffer = request.getParameter("address" + 4 + "line4"+parseItem);
	    if (buffer != null ) {
	      this.setStreetAddressLine4(buffer);
	    }
	    else
	    {
	    	  this.setStreetAddressLine4("");
	    }
	    buffer = request.getParameter("address" + 4 + "city"+parseItem);
	    if (buffer != null ) {
	      this.setCity(buffer);
	    }
	    else
	    ...
	    //this.setCountry(request.getParameter("address" + 4 + "country"+parseItem));
	    

	    this.setCountry("Italia");
	    buffer = request.getParameter("address" + 4 + "state"+parseItem);
	    String buffer2 = request.getParameter("address4state"+parseItem);  //aggiunta da d.dauria per prova
	    ...
	    buffer = request.getParameter("address" + 4 + "otherState"+parseItem);
	    if (buffer != null ) {
	    ...
	    buffer = request.getParameter("address" + 4 + "zip"+parseItem);
	    if (buffer != null ) {
	      this.setZip(buffer);
	    }
	    else
	    ...
	    buffer = request.getParameter("address" + 4 + "county"+parseItem);
	    if (buffer != null) {
	    ...
	    buffer = request.getParameter("address" + 4 + "latitude"+parseItem);
	    if (buffer != null ) {
	    ...
	    buffer = request.getParameter("address" + 4 + "longitude"+parseItem);
	    if (buffer != null ) {
	    ...
	    if (request.getParameter("address" + 4 + "delete"+parseItem) != null) {
	      String action = request.getParameter("address" + 4 + "delete"+parseItem)
	          .toLowerCase();
	      if (action.equals("on")) {
  ...
  public void buildRecord(HttpServletRequest request, int parseItem) {
    ...
    if (request.getParameter("address" + parseItem + "id") != null && ! "".equals(request.getParameter("address" + parseItem + "id"))) {
    ...
    String buffer2 = request.getParameter("address1state");  //aggiunta da d.dauria per prova
    if (buffer != null && !"".equals(buffer.trim())) {
      this.setState(buffer);
    }
    else
   ...
   * @since 1.8
   */
  public void buildRecordAddressSoggettoFisico(HttpServletRequest request) {
   // this.setType(request.getParameter("address" + parseItem + "type"));
 /*   if (request.getParameter("address" + parseItem + "id") != null && ! "".equals(request.getParameter("address" + parseItem + "id"))) {
      this.setId(request.getParameter("address" + parseItem + "id"));
    }*/
    ...
    String buffer = null;
    buffer = request.getParameter("addressLegaleLine1");
    ...
    buffer = request.getParameter("addressLegaleLine2");
    if (buffer != null && !"".equals(buffer.trim())) {
      this.setStreetAddressLine2(buffer);
    }
    else
    ...
    buffer = request.getParameter("addressLegaleLine3");
    if (buffer != null && !"".equals(buffer.trim())) {
      this.setStreetAddressLine3(buffer);
    }
    else
    ...
    buffer = request.getParameter("addressLegaleLine4");
    if (buffer != null && !"".equals(buffer.trim())) {
      this.setStreetAddressLine4(buffer);
    }
    else
    ...
    buffer = request.getParameter("addressLegaleCity");
    if (buffer != null ) {
      this.setCity(buffer);
    }
    else
    ...
    this.setCountry(request.getParameter("addressLegaleCountry"));
    buffer = request.getParameter("addressLegalestate");
    String buffer2 = request.getParameter("address1state");  //aggiunta da d.dauria per prova
    if (buffer != null && !"".equals(buffer.trim())) {
      this.setState(buffer);
    }
    ...
    buffer = request.getParameter("addressLegaleOtherState");
    if (buffer != null && !"".equals(buffer.trim())) {
      this.setOtherState(buffer);
    }
    else
    ...
    buffer = request.getParameter("addressLegaleZip");
    if (buffer != null && !"".equals(buffer.trim())) {
      this.setZip(buffer);
    }
    else
    ...
    buffer = request.getParameter("addressLegaleCounty");
    if (buffer != null && !"".equals(buffer.trim())) {
      this.setCounty(buffer);
    }
    else
    ...
    buffer = request.getParameter("addressLegaleLatitude");
    if (buffer != null && !"".equals(buffer.trim())) {
      this.setLatitude(buffer);
    }
    else
    ...
    buffer = request.getParameter("addressLegaleLongitude");
    if (buffer != null && !"".equals(buffer.trim())) {
      this.setLongitude(buffer);
    }
    else
    ...
    if (request.getParameter("addressLegaleDelete") != null) {
      String action = request.getParameter("addressLegaleDelete")
          .toLowerCase();
      if (action.equals("on")) {
        this.setEnabled(false);
      }
